...
// Output directory
string odir = output_dir;
//char* odir = const_cast<char*>(output_dir.c_str());
...
/// Output to file Special and Ordinary faces order :: tess - means "numeration of Faces start with 1 instead of 0 like in the NEPER output"
    ...
    if (OutSfaces_file) { //        OutSGBfile << "Global numbers (in DCC) of special grain boundaries with the fraction " << special_faces_sequence.size()/ CellNumbs.at(2) << endl;
        for (auto rvit: special_cells_design.at(0)) OutSfaces_file << rvit + 1 << endl; /// vit + 1 !!! for compatibility with the Neper output
        cout << "(" << output_counter++ << ")  " << "Random special faces_sequence has been successfully written in " << output_dir + "RandomSpecialFaces.txt"s << endl;
