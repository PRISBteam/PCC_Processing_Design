...
std::vector<unsigned int> State_p_vector, State_f_vector, State_e_vector, State_n_vector; // based on each sequences in the list, the State_<*>_vector's of special cells can be calculated, including State_n_vector, State_e_vector, State_f_vector and State_p_vector
std::vector<vector<int>> Configuration_State(dim+1); //  is the list of all mentioned below State_<*>_vectors as a possible output of the Processing module
...
// III: DCC_Characterisation module
        if (ConfigVector.at(3) == 1) {
            ...
            //PCC_StructureCharacterisation(State_sVector, special_faces_sequence, crack_faces_sequence, ConfigVector);
...
/// VI: PCC_Writing module
        if (ConfigVector.at(6) == 1) { // simply output ON/OFF for the PCC_Writer module on the screen
            ...
            //PCC_Writer(special_faces_sequence, kface_sequence);
