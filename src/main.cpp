...
/// Read simulation configuration from file :: the number of special face types and calculating parameters. Then Output of the current configuration to the screen and *.log file.
    string main_type; /// SIMULATION MODE(LIST) or 'SIMULATION MODE(TASK) :: This define the global simulation mode: 'LIST' for the "list" of modules implementing one by one (if ON) and 'TASK' for the user-defined task scripts with modules and functions included from the project's libraries

    vector<int> ConfigVector = config_reader_main(source_path, source_dir, output_dir, main_type, e_mode);
// ConfigVector (../config/main.ini) contains ALL the control variables needed for the program execution

/// Useful off-streams cleaning for data output
    ...
    if(dim != 1 && dim != 2 && dim != 3) { cout << "Wrong dimension ERROR! Please change 'dim' parameter in ../config/mail.ini file to 1, 2, or 3" << endl; Out_logfile_stream << "Wrong dimension ERROR! Please change 'dim' parameter in ../config/mail.ini file to 1, 2, or 3" << endl; exit(1); }
...
string ssd0, ssd1, ssd2, ssd3, ssd4, ssd5, ssd6, ssd7, ssd8, ssd9, ssd10, ssd11, ssd12, ssd13, ssd14; // PCC matrices

if (is_file_exists(source_dir + "algebraic/A0.txt"s) && is_file_exists(source_dir + "algebraic/A1.txt"s) && is_file_exists(source_dir + "algebraic/B1.txt"s) && dim >= 0)
    ssd0 = source_dir + "algebraic/A0.txt"s, ssd1 = source_dir + "algebraic/A1.txt"s, ssd4 = source_dir + "algebraic/B1.txt"s; // 1D
if (is_file_exists(source_dir + "/algebraic/A2.txt"s) && is_file_exists(source_dir + "algebraic/B2.txt"s) && dim >= 2)
    ssd2 = source_dir + "algebraic/A2.txt"s, ssd5 = source_dir + "algebraic/B2.txt"s; // 2D
if (is_file_exists(source_dir + "/algebraic/A3.txt"s) && is_file_exists(source_dir + "algebraic/B3.txt"s) && dim == 3)
    ssd3 = source_dir + "algebraic/A3.txt"s,  ssd6 = source_dir + "algebraic/B3.txt"s; // 3D
if (dim > 3) { cout << "INPUT DATA ERROR (!) dim > 3 as it specified in the ../config/main.ini file. Please, make it equal to 1, 2 or 3." << endl; Out_logfile_stream << "INPUT DATA ERROR (!) dim > 3 in the ../config/main.ini file. Please, make it equal to 1, 2 or 3." << endl; exit(1);}
...
/// Vector with rhe numbers of PCC k-cells for k\in{0,1,2,3} from file
    if (is_file_exists(source_dir + "/combinatorial/number_of_cells.txt"s)) {
        string ncells = source_dir + "/combinatorial/number_of_cells.txt"s;
...
cout << "=====================================================================================" << endl;  Out_logfile_stream << "==========================================================================================================================================================================" << endl;
    ...
    else cout << "ERROR: The file " << source_dir + "/combinatorial/number_of_cells.txt"s << " does not exists (!)" << endl;

/// Initial state::
    if(dim < 3) { // Does not exist in 2D: CellNumbs.at(3)
        State_p_vector.resize(0,0); State_pfracture_vector.resize(0, 0);
        if (dim == 1)
            State_f_vector.resize(0, 0); State_ffracture_vector.resize(0, 0);
    } else {
        State_p_vector.resize(CellNumbs.at(3), 0); State_pfracture_vector.resize(CellNumbs.at(3), 0);
        State_f_vector.resize(CellNumbs.at(2), 0); State_ffracture_vector.resize(CellNumbs.at(2), 0);
    }
    State_e_vector.resize(CellNumbs.at(1), 0); State_n_vector.resize(CellNumbs.at(0), 0);
    State_efracture_vector.resize(CellNumbs.at(1), 0); State_nfracture_vector.resize(CellNumbs.at(0), 0);

    if (dim == 1) { // 1D case
    Configuration_State.resize(2); Configuration_State = {State_n_vector, State_e_vector};
    Configuration_cState.resize(2); Configuration_cState = {State_nfracture_vector, State_efracture_vector};
    }
if (dim == 2) { // 2D case
    Configuration_State.resize(3); Configuration_State = {State_n_vector, State_e_vector, State_f_vector};
    Configuration_cState.resize(3); Configuration_cState = {State_nfracture_vector, State_efracture_vector, State_ffracture_vector};
}
else { // 3D case
    Configuration_State.resize(4); Configuration_State = {State_n_vector, State_e_vector, State_f_vector, State_p_vector};
    Configuration_cState.resize(4); Configuration_cState = {State_nfracture_vector, State_efracture_vector, State_ffracture_vector, State_pfracture_vector};
