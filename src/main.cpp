...
string input_dir, output_dir; // input and output directories read from the file config.txt file
...
#include "PCC_Objects/PCC_Objects.h"
...
/* Output: module generates an s_sequences list with the sequences of k-cells possessing "special" IDs (1) generated: corresponding to different generation principles (random, maximum entropy, etc.) and (2) imposed by high-order k-cells on their (k-1)-cells boundaries */
...
#include "PCC_Processing/PCC_Processing.h"
...
#include "PCC_Kinetic/PCC_Kinetic.h"
...
#include "PCC_Section/PCC_Subcomplex.h"
...
#include "PCC_Multiphysics/PCC_Multiphysics.h"
...
/// File path to the configuration profile (MainPath is the path to the code's main directory)
    string MainPath = "../";
    string config = MainPath + "config_main.txt"s; char* confpath = const_cast<char*>(config.c_str()); // file path with config_main.txt
...
/// Read simulation configuration from file :: the number of special face types and calculating parameters. Then Output of the current configuration to the screen
    ...
    string M_type; // 'L' or 'T' :: This char define the global simulation mode: 'L' for the "list" of modules implementing one by one (if ON) and 'T' for the user-defined task scripts with modules and functions from the project's libraries
...
/// ConfigVector contains all the control variables of the program readed from the config.txt
    ...
    vector<double> ConfigVector = config_reader_main(confpath, S_type, P_type, K_type, input_dir, output_dir);
    char* indir = const_cast<char*>(input_dir.c_str()); // const_cast for input directory //    char* odir = const_cast<char*>(output_dir.c_str()); // const_cast for output directory
...
/// They can be obtain by the PCC Generator tool (https://github.com/PRISBteam/Voronoi_PCC_Analyser) based on the Neper output (*.tess file) (https://neper.info/) or PCC Structure Generator tool (https://github.com/PRISBteam/PCC_Structure_Generator) for plasticity problems
    string ssd0 = input_dir + "A0.txt"s, ssd1 = input_dir + "A1.txt"s, ssd2 = input_dir + "A2.txt"s, ssd3 = input_dir + "A3.txt"s, ssd4 = input_dir + "B1.txt"s, ssd5 = input_dir + "B2.txt"s, ssd6 = input_dir + "B3.txt"s,
            seeds = input_dir + "seeds.txt"s; //, NewSeeds = input_dir + "NewSeeds/NewSeeds.txt"s;
//The next line just a technical procedure string to char arrays transformation needed to use them as the function arguments
    ...
    if (is_file_exists(input_dir + "voro_Ncells.txt"s)) { ncells = input_dir + "voro_Ncells.txt"s; number_of_cells = const_cast<char*>(ncells.c_str()); }
    else if (is_file_exists(input_dir + "delau_Ncells.txt"s)) { ncells = input_dir + "delau_Ncells.txt"s; number_of_cells = const_cast<char*>(ncells.c_str()); }
    else { ncells = input_dir + "number_of_cells.txt"s; number_of_cells = const_cast<char*>(ncells.c_str()); }
...
//REPAIR    cout << "Size of grain_seeds: " << gs_size << " CellNumbs.at(3): " << CellNumbs.at(3) << endl;
    ...
    string FSpath_string = input_dir + "face_seeds.txt"s;
...
//REPAIR
    ...
    */
    ...
    string VCpath_string = input_dir + "vertex_seeds.txt"s;
...
//REPAIR    cout << "vertex_coordinates_vector size: " << vertex_coordinates_vector.size() << endl; //exit(0); exit(0);
    ...
    /// All grain volumes
    string GVpath_string = input_dir + "grain_volumes.txt"s;
    ...
    /// All face areas
    string GBApath_string = input_dir + "face_areas.txt"s;
