...
map<unsigned int, unsigned int> EdgesStat(std::vector<unsigned int> const& CellNumbs, unsigned long numerator, map<unsigned int,unsigned int> SpecialCellMap, Eigen::SparseMatrix<double> const& FES, char* output_dir, double special_faces_fraction)
{
...
// (!) log2 means binary (or base-2) logarithm and we use "-" for fractions to make the value positive
    ...
    Face_Entropy_Median = -(1.0/4.0)*log2(j0*j1*j2*j3);
    // Skrew part (divergence from the uniform distribution -> S_max) in the entropy decomposition
    vector<double> j_types_fractions = {j0, j1, j2, j3};
    for (int j = 0; j < 4; j++)
        for (int i = 0; i < j; i++)
            Face_Entropy_Skrew += -(1.0/4.0)*(j_types_fractions[i]-j_types_fractions[j])*log2(j_types_fractions[i]/j_types_fractions[j]);
