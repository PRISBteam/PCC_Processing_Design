...
cout << "START of DCC Structure Characterisation Module" << endl;
    ...
    SpMat  SpAM_SpecFaces(s_faces_sequence.size(), s_faces_sequence.size()), SFace_Laplacian(s_faces_sequence.size(), s_faces_sequence.size()),
            Sym_SFace_Laplacian(s_faces_sequence.size(), s_faces_sequence.size()), RW_SFace_Laplacian(s_faces_sequence.size(), s_faces_sequence.size()),
...
/// Incidence sparse matrix for Edges and Nodes /// Incidence sparse matrix for Faces and Edges /// Incidence sparse matrix for Grains and Faces
    SpMat ENS(CellNumbs.at(1),CellNumbs.at(0)), FES(CellNumbs.at(2),CellNumbs.at(1)), GFS(CellNumbs.at(3),CellNumbs.at(2));
    ENS = SMatrixReader(paths.at(4), (CellNumbs.at(1)), (CellNumbs.at(0))); //all Nodes-Edges
    FES = SMatrixReader(paths.at(5), (CellNumbs.at(2)), (CellNumbs.at(1))); //all Edges-Faces
    GFS = SMatrixReader(paths.at(6), (CellNumbs.at(3)), (CellNumbs.at(2))); //all Faces-Grains
...
OutFaceFile << endl << "max Special Faces fraction is equal to\t " << MAX_sfaces_fraction  << endl << endl;                 // Output of special_faces_fraction to files
...
OutRWFLfile << "Global numbers (in DCC) of special grain boundaries with the fraction " << MAX_sfaces_fraction << endl;
