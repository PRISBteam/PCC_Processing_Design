...
//        cout << " x " << x << " Len " << Len << endl;  exit(0);
        ...
        double r2p = std::pow(x, 2.0) + std::pow(y, 2.0) - std::pow(Len, 2.0)/4.0 - D_val;
        double r2m = std::pow(x, 2.0) - std::pow(y, 2.0) - std::pow(Len, 2.0)/4.0 + D_val;
        double x2p = std::pow(x, 2.0) + std::pow(Len, 2.0)/4.0 - D_val;
        double x2m = std::pow(x, 2.0) - std::pow(Len, 2.0)/4.0 + D_val;
        double y2p = std::pow(y, 2.0) + std::pow(Len, 2.0)/4.0 - D_val;
        double y2m = std::pow(y, 2.0) - std::pow(Len, 2.0)/4.0 + D_val;

        double Pp = std::sqrt(std::pow(r2m, 2.0) + 4.0 * std::pow(x, 2.0) * std::pow(y,2)); //pow((y - Len/2.0), 2)); //        double Pp = std::sqrt(pow(r2m, 2.0) + 4.0 * pow(x, 2.0) * pow((y - (Len/2.0)),2)); //pow((y - Len/2.0), 2));
        ...
        Sxx_crack = Sigm * (Pp * r2p * (abs(y) * Qm - abs(x) * Qp) -
                            abs(x) * Qp * (std::pow(y, 4.0) - 2.0 * x2p * std::pow(y, 2.0) - 3.0 * std::pow(x2m, 2.0)) +
                            abs(y) * Qm * (std::pow(y, 4.0) + 6.0 * x2p * std::pow(y, 2.0) + 5.0 * std::pow(x2m, 2.0))) /
                    (2.0 * std::sqrt(2.0) * std::pow(Pp, 3.0)); // + external_stress(0,0)

        Syy_crack = Sigm * (Pp * r2p * (abs(x) * Qp - abs(y) * Qm) +
                            abs(x) * Qp * (5.0 * std::pow(y, 4.0) + 6.0 * x2p * std::pow(y, 2.0) + std::pow(x2m, 2.0)) +
                            abs(y) * Qm * (3.0 * std::pow(y, 4.0) + 2.0 * x2p * std::pow(y, 2.0) - std::pow(x2m, 2.0))) /
                    (2.0 * std::sqrt(2.0) * std::pow(Pp, 3.0)); ///+ external_stress(1,1); // + external_stress(1,1)

        Szz_crack = nu * (Sxx_crack + Syy_crack);

        Sxy_crack = Sigm * sign(x * y) * (Pp * r2p * (abs(x) * Qm + abs(y) * Qp) -
                                          abs(x) * Qm * (3.0 * std::pow(y, 4.0) + 2.0 * x2p * std::pow(y, 2.0) - std::pow(x2m, 2.0)) +
                                          abs(y) * Qp * (std::pow(y, 4.0) - 2.0 * x2p * std::pow(y, 2.0) - 3.0 * std::pow(x2m, 2.0))) /
                    (2.0 * std::sqrt(2.0) * std::pow(Pp, 3.0)); // + external_stress(2,0)
