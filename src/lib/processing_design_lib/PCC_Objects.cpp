...
Config::main_configuration Config::Get_main_config() const {
    return main_config;
}
Config::subcomplex_configuration Config::Get_subcomplex_config() const {
    return subcomplex_config;
}

//main
void Config::Set_main_type(std::string &main_type_str){
    main_config.main_type = main_type_str;
}
...
int Config::Get_dim() const {
    ...
    std::string Config::Get_source_dir() const {
        return Get_main_config().pcc_source_dir;
    }; //!@return source_dir

    std::string Config::Get_output_dir() const {
         return Get_main_config().output_dir;
    }; //!@return output_dir

    std::string Config::Get_pcc_standard() const {
        return Get_main_config().pcc_standard;
    }; //!@return pcc_standard
    ...
    std::string Config::Get_main_type() const {
         return Get_main_config().main_type;
    }; //!@return output_dir
  ...
  if (main_configuration.Get_main_config().pcc_standard == "pcc1s") {
...
//  PCC matrices, metrices and coordinates
        ...
        if (is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "algebraic/A0.txt"s)
                && is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "algebraic/A1.txt"s) &&
            is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "algebraic/B1.txt"s) && config_dim >= 0)
            ssd0 = main_configuration.Get_main_config().pcc_source_dir + "algebraic/A0.txt"s,
            ssd1 = main_configuration.Get_main_config().pcc_source_dir + "algebraic/A1.txt"s,
            ssd4 = main_configuration.Get_main_config().pcc_source_dir + "algebraic/B1.txt"s; // 1D
        else {
        ...
        if (is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "/algebraic/A2.txt"s)
                && is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "algebraic/B2.txt"s) &&
                config_dim >= 2)
            ssd2 = main_configuration.Get_main_config().pcc_source_dir + "algebraic/A2.txt"s,
            ssd5 = main_configuration.Get_main_config().pcc_source_dir + "algebraic/B2.txt"s; // 2D
        else {
        ...
        if (is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "/algebraic/A3.txt"s)
                && is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "algebraic/B3.txt"s) &&
                config_dim == 3)
            ssd3 = main_configuration.Get_main_config().pcc_source_dir + "algebraic/A3.txt"s,
            ssd6 = main_configuration.Get_main_config().pcc_source_dir + "algebraic/B3.txt"s; // 3D
        else {
...
/// ---> edge lengths HERE (!) // see next 'ssd14 = source_dir + "edge_lengths.txt"s;' and replace with 'ssd6'
        if (is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "measures/face_areas.txt"s) && config_dim >= 2)
            ssd7 = main_configuration.Get_main_config().pcc_source_dir + "measures/face_areas.txt"s; // 2D face_areas
        else
        ...
        if (is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "measures/polyhedron_volumes.txt"s) && config_dim == 3)
            ssd8 = main_configuration.Get_main_config().pcc_source_dir + "measures/polyhedron_volumes.txt"s; // 3D polyhedron_volumes
        else
...
// In the for of vector<tuple<double, double, double>> vertex_coordinates_vector, edge_coordinates_vector, face_coordinates_vector, grain_coordinates_vector;
        if (is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "coordinates/polyhedron_seeds.txt"s) && config_dim == 3)
            ssd9 = main_configuration.Get_main_config().pcc_source_dir + "coordinates/polyhedron_seeds.txt"s; // grain (polyhedron) seeds
        else
        ...
        if (is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "coordinates/vertex_seeds.txt"s) && config_dim >= 1)
            ssd10 = main_configuration.Get_main_config().pcc_source_dir + "coordinates/vertex_seeds.txt"s; // vertex coordinates
        else
        ...
        if (is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "other/face_normals.txt"s) && config_dim >= 2)
            ssd11 = main_configuration.Get_main_config().pcc_source_dir + "other/face_normals.txt"s; // face normal vectors
        else
        ...
        if (is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "coordinates/edge_seeds.txt"s) && config_dim >= 1)
            ssd12 = main_configuration.Get_main_config().pcc_source_dir + "coordinates/edge_seeds.txt"s; // edge barycentres coordinates
        else
        ...
        if (is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "coordinates/face_seeds.txt"s) && config_dim >= 2)
            ssd13 = main_configuration.Get_main_config().pcc_source_dir + "coordinates/face_seeds.txt"s; // face barycentres coordinates
        else
...
/// Vector with rhe numbers of PCC k-cells for k\in{0,1,2,3} from file
        if (is_file_exists(main_configuration.Get_main_config().pcc_source_dir + "/combinatorial/number_of_cells.txt"s)) {
            std::string ncells = main_configuration.Get_main_config().pcc_source_dir + "/combinatorial/number_of_cells.txt"s;
        ...
        else cout << "ERROR: The file " << main_configuration.Get_main_config().pcc_source_dir + "/combinatorial/number_of_cells.txt"s
...
std::vector <tuple<double, double, double>> Subcomplex::Get_sub_cfaces_coord(void) const {
    ...
    /// Geometry
    void Subcomplex::Set_sub_face_coordinates(std::vector<tuple<double, double, double>> &new_sub_face_coordinates){
        sub_face_coordinates = new_sub_face_coordinates; }
    std::vector<tuple<double, double, double>> Subcomplex::Get_sub_face_coordinates(void) const {
    return sub_face_coordinates; }
...
std::set <unsigned int> Macrocrack::Get_sfaces_set() const {
    if (plane_subcomplex.Get_sub_sfaces_set().size() > 0)
        return plane_subcomplex.Get_sub_sfaces_set();
    else {
        cout << "Caution! sub_sfaces_sequence = 0! Please add it to the corresponding subcomplex/crack"s << endl;
        return {0}; }
}
std::set <unsigned int> Macrocrack::Get_internal_sfaces_set() const {
    if (plane_subcomplex.Get_internal_sub_sfaces_set().size() > 0)
        return plane_subcomplex.Get_internal_sub_sfaces_set();
    else {
        cout << "Caution! sub_sfaces_sequence = 0! Please add it to the corresponding subcomplex/crack"s << endl;
        return {0}; }
}
...
std::vector <tuple<double, double, double>> Macrocrack::Get_internal_sfaces_coordinates(void) const {
    return plane_subcomplex.Get_sub_internal_face_coordinates();
}
...
void CellEnergies::Set_homogeneous_elastic_energy(std::tuple<double, double, double> &sample_dimensions, double &von_Mises_elastic_stress, Material &matrix_material) { // [J]
    homogeneous_elastic_energy = std::pow(von_Mises_elastic_stress,2)*std::get<0>(sample_dimensions)*std::get<1>(sample_dimensions)*std::get<2>(sample_dimensions)/ (2.0*matrix_material.Get_Young_modulus());
    }
