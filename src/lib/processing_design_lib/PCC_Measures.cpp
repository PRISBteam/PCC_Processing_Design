...
{
// Output of the model
    std::vector<int> TJsTypes(CellNumbs.at(1 + (PCC_dimension - 3)), 0); // CellNumbs.at(1) is the number of Edges

// Obtaining Faces (coloumns) - Edges (rows) Incidence matrix B2 using the file paths.at(5 + (dim - 3))
    SpMat FES = SMatrixReader(paths_to_PCC_matrices.at(5 + (PCC_dimension - 3)), CellNumbs.at(1 + (PCC_dimension - 3)), CellNumbs.at(2 + (PCC_dimension - 3))); // Edges-Faces sparse incidence matrix

    for (auto f : special_face_sequence) // loop over all Special Faces
        for(int e = 0; e < CellNumbs.at(1 + (PCC_dimension - 3)); ++e) // loop over all Edges
