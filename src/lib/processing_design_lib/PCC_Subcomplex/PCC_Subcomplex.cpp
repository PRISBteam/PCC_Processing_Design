...
extern ofstream subcomplex_logfile_stream;
...
/// The source directory and simulation type from file ..\config\subcomplex.ini
    ...
    bool is_log_file_output = 0;

    /// Reading of the configuration from the '../config/subcomplex.ini' file
    config_reader_subcomplex(S_type, plane_orientation, cut_length, grain_neighbour_orders, is_log_file_output); // void function
...
//        subcomplex_logfile_stream.open(output_dir + "cpdlog_subcomplex.log"s,
        ...
        subcomplex_logfile_stream << " Execution year - " << 1900 + datetime.tm_year << "; Month - " << datetime.tm_mon
                                  << "; Day - " << datetime.tm_mday << "; Time - " << datetime.tm_hour << ":"
                                  << datetime.tm_min << "." << endl << endl;
    }
// -------------------------------------------------
...
/// Setting all quantities to the subcomplex new_subPCC with id = 0
    ...
    new_cut.Set_sub_internal_face_coordinates(internal_faces_coordinates);
...
/// The source directory and simulation type from file ..\config\subcomplex.ini
    ...
    /// Reading of the configuration from the '../config/subcomplex.ini' file
    config_reader_subcomplex(S_type, plane_orientation, cut_length, grain_neighbour_orders); // void function
...
/// Common face coordinates
    for(unsigned int fnumber = 0; fnumber < CellNumbs.at(2); ++fnumber) {
        if (std::find(doubled_sub_faces_sequence.begin(), doubled_sub_faces_sequence.end(), fnumber) != doubled_sub_faces_sequence.end())
            internal_faces_coordinates.push_back(find_aGBseed(fnumber, paths_to_PCC_matrices, CellNumbs, polytope_coordinates_vector)); // tuple<double, double, double> NewSeed_coordinates = make_tuple(0, 0, 0);
    }
