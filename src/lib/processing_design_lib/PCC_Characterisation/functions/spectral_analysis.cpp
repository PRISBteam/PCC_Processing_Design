...
/// Special face-related PCC elements
    ...
    std::vector<int> node_ori_vector(CellNumbs.at(0),0), edge_ori_vector(CellNumbs.at(1 + (PCC_dimension - 3)), 0),
            face_ori_vector(CellNumbs.at(2 + (PCC_dimension - 3)), 0), grain_ori_vector(CellNumbs.at(3 + (PCC_dimension - 3)), 0); // number of different sequences based on the face design vector
...
//    unsigned int node_numerator = 0, edge_numerator = 0, grain_numerator = 0;
    ...
    for (auto  itf = face_sequence_vector.begin(); itf != face_sequence_vector.end(); ++itf) {
        // grains
        for (unsigned int g = 0; g < CellNumbs.at(3 + (PCC_dimension - 3)); ++g) //  Loop over all the Grains in the PCC
...
//                        grain_fsequence_vector.push_back(g);
        ...
        // edges
        for (unsigned int e = 0; e < CellNumbs.at(1 + (PCC_dimension - 3)); ++e) //  Loop over all the Edges in the PCC
...
//        cout << endl;
    ...
    if (PCC_dimension == 3) { // nodes - only for the 3D case
        /// faces loop
