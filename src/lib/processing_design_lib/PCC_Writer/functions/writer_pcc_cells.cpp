...
///======================================================================================================================================///
/** The library contains functions for the formatted output of a variety of data types generated by other modules and associated with   ///
 * the PCC's p-cells, their sequences and energies.                                                                                    **/
///===================================================================================================================================///
#include <iostream>
#include <fstream>
#include <string>
...
extern Config initial_configuration, configuration; // Configuration is an object of the 'Config' class described in the Objects.cpp project library

#include "writer_pcc_cells.h"
...
/*
    ...
    // special INDUCED cell sequences
        ...
        writer_logfile_stream << "(" << output_counter << ")  " << "Random special k-cells for all k = {0,1,2,3} sequences has been successfully written in " << seq_odir << endl;
 ...
 */
    ...
    if (Out_scells_file) {
        ...
        writer_logfile_stream << "(" << output_counter << ")  " << "Random special k-cells for all k = {0,1,2,3} sequences has been successfully written in " << des_odir << endl << endl;
        Out_scells_file.close();
    ...
    if (Out_agglomerations_file) {
        ...
        writer_logfile_stream << "(" << output_counter << ")  " << "Agglomeration cells for all k = {0,1,2,3} sequences has been successfully written in " << des_odir << endl << endl;
...
// Offstreams
    ...
    config_reader_multiphysics(configuration);

    std::string Mid_matrix = configuration.Get_multiphysics_matrixMaterial_id();
    std::string Mid_inclusion = configuration.Get_multiphysics_inclusionMaterial_id();
    std::tuple<double, double, double> sample_dimensions = configuration.Get_multiphysics_sample_dimensions();
    double tau = configuration.Get_multiphysics_time_scale();
    Eigen::MatrixXd external_stress_tensor(3,3);
    external_stress_tensor = configuration.Get_multiphysics_external_stress_tensor();
...
//        Out_e_elastic_energy_file << get<0>(edge_coordinates_vector[ee])*std::get<0>(sample_dimensions)*pow(10.0,6) << "\t" << get<1>(edge_coordinates_vector[ee])*std::get<1>(sample_dimensions)*pow(10.0,6) << "\t" << get<2>(edge_coordinates_vector[ee])*std::get<2>(sample_dimensions)*pow(10.0,6) << endl;
    ...
    if(face_coordinates_vector.size() == 0)
        face_coordinates_vector = Tuple3Reader(paths_to_PCC_matrices.at(13)); // face barycentres
...
/// (1) Configuration etropies:  / full / mean / skrew /
    ...
    if (Out_entropies_file) {
        ...
        writer_logfile_stream << "(" << output_counter << ")  " << "Configuration edge entropies has been successfully written in " << entropies_odir << endl;
...
/// (2) Edge fractions:  / j0 / j1 / j2 / j3 /
    ...
    if (Out_jf_file) {
        ...
        writer_logfile_stream << "(" << output_counter << ")  " << "Special edge fractions has been successfully written in   " << ffrac_odir << endl;
        Out_jf_file.close();
    ...
    if (Out_df_file) {
        ...
        writer_logfile_stream << "(" << output_counter << ")  " << "Special edge degree fractions has been successfully written in   " << dfrac_odir << endl;
        Out_df_file.close();
...
//crystalline
    ...
    writer_logfile_stream << "(" << output_counter << ")  " << "Analytical Special Edges and Special Edges Degree fractions has been successfully written in  " << antjsrand_odir << endl << antjscryst_odir << endl << antjdsrand_odir << endl << antjdscryst_odir << endl;
...
// configuration edge entropies vectors
    ...
    writer_logfile_stream << "(" << output_counter << ")  " << "Analytical Configuration Edge Entropies (random and crystallographic)) has been successfully written in " << anentropiesrand_odir << endl << anentropiescryst_odir << endl;
...
void PCC_Laplacians_Writer(ProcessedComplex &pcc_processed, int &output_counter) {
    ...
    writer_logfile_stream << "(" << output_counter << ")  " << "Betti numbers of the special cell structure evolution has been successfully written in " << betti_odir << endl;
