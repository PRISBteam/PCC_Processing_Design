...
// induced structure
    string ctypes_number_string;
    if (processing_ini.has("faces")) {
        auto& collection = processing_ini["faces"];
        if (collection.has("crack_types_number"))
            ctypes_number_string = processing_ini.get("faces").get("crack_types_number");
    }
    ...
    if (processing_ini.has("faces")) {
        ...
        if (collection.has("cfmax_fraction"))
            c_max = processing_ini.get("faces").get("cfmax_fraction");
    }
    if (stoi(ctypes_number_string) > 0) max_cfractions_vectors.at(2).push_back(stod(c_max)); // 2 - faces
...
// refill 0s
    ...
    if (ctypes_number_string != "0") {
        cout << "Processing c_type:\t"s << ctype_vector.at(2) << endl;
        cout << "Number of face crack types:\t"s << ctypes_number_string << endl;
...
// refill 0s
    ...
    if (ctypes_number_string != "0") {
        Out_logfile_stream << "Processing c_type:\t"s << ctype_vector.at(2) << endl;
        Out_logfile_stream << "Number of face crack types:\t"s << ctypes_number_string << endl;
