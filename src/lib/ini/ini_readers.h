...
/// Polyhedrons
    ...
    if (char_ini.has("polyhedrons_lab")) {
        auto& collection = char_ini["polyhedrons_lab"];
        if (collection.has("omega"))
        {
            charlabs_polyhedrons.push_back(stoi(char_ini.get("polyhedrons_lab").get("omega")));
        } }
...
/// Faces
    ...
    if (char_ini.has("faces_lab")) {
        ...
        if (collection.has("sigma"))
        {
            charlabs_faces.push_back(stoi(char_ini.get("faces_lab").get("sigma")));
        } }

    if (char_ini.has("faces_lab")) {
        auto& collection = char_ini["faces_lab"];
        if (collection.has("chi"))
        {
            charlabs_faces.push_back(stoi(char_ini.get("faces_lab").get("chi")));
        } }
...
/// Edges
    ...
    if (char_ini.has("edges_lab")) {
        ...
        {
            charlabs_edges.push_back(stoi(char_ini.get("edges_lab").get("j_fractions"))); // [3]
        } }
    ...
    if (char_ini.has("edges_lab")) {
        ...
        {
            charlabs_edges.push_back(stoi(char_ini.get("edges_lab").get("d_fractions"))); // [3]
        } }
...
int isBetti = 0; // [7] Laplacians lab
int isOmega = 0, isSigma = 0, isChi = 0; // [8], [9], [10]
...
// IV Polyhedrons
    if (writer_ini.has("entropic_polyhedrons")) {
        auto& collection = writer_ini["entropic_polyhedrons"];
        if (collection.has("omega"))
        {
            isOmega = stoi(writer_ini.get("entropic_polyhedrons").get("omega"));
        } }
    writer_specifications.push_back(isOmega); // [8]

// V Faces
    if (writer_ini.has("entropic_faces")) {
        auto& collection = writer_ini["entropic_faces"];
        if (collection.has("sigma"))
        {
            isSigma = stoi(writer_ini.get("entropic_faces").get("sigma"));
        } }
    writer_specifications.push_back(isSigma); // [9]

    if (writer_ini.has("entropic_faces")) {
        auto& collection = writer_ini["entropic_faces"];
        if (collection.has("chi"))
        {
            isChi = stoi(writer_ini.get("entropic_faces").get("chi"));
        } }
    writer_specifications.push_back(isChi); // [10]
