...
/// *                                                                                                                                                    * ///
vector<int> Edge_types_byFaces(std::vector<unsigned int> const &CellNumbs, std::vector<unsigned int> const &special_face_sequence, std::vector<double> &j_fractions, std::vector<double> &d_fractions)
{
// Output of the model
...
for (auto vit: special_face_sequence) // loop over all Special Faces
    for(int e = 0; e < CellNumbs.at(1 + (dim - 3)); ++e) // loop over all Edges
        if (FES.coeff(e, vit) == 1) TJsTypes.at(e)++;
...
// (!) log2 means binary (or base-2) logarithm and we use "-" for fractions to make the value positive
    ...
    for (int k = 0; k < 3; ++k) {
        if (j_fractions.at(0) != 1.0) d_fractions.at(k) = j_fractions.at(k + 1) / (1.0 - j_fractions.at(0));
