...
/// *                                                                                                                                                    * ///
vector<int> Edge_types_byFaces(std::vector<unsigned int> const &CellNumbs, std::vector<unsigned int> const &special_cells_sequence, std::vector<double> &j_fractions, std::vector<double> &d_fractions)
{
// Output of the model
...
for (auto vit: special_cells_sequence) // loop over all Special Faces
...
// (!) log2 means binary (or base-2) logarithm and we use "-" for fractions to make the value positive
    ...
    for (int k = 1; k < 4; ++k)
        if (j_fractions.at(0) != 1) d_fractions.at(k) = j_fractions.at(k)/ (1.0 - j_fractions.at(0));
