...
//std::vector <unsigned int> PCC_Kinematic_cracking(std::vector<unsigned int> &s_faces_sequence, Eigen::SparseMatrix<double> const& AFS, Eigen::SparseMatrix<double> const& FES) {
std::vector <unsigned int> PCC_Kinematic_cracking(int cell_type, std::vector<unsigned int> &s_faces_sequence, std::vector<vector<int>> &Configuration_cState, std::vector<vector<double>> const &max_cfractions_vectors) {
    ...
    std::vector <double> lsc_inclusion_energy(CellNumbs.at(cell_type), 0), lsc_crack_energy(CellNumbs.at(cell_type), 0); // energies related with the local stress concentrators
 ...
 // deleting of all faces contain inclusions
    for (auto ks : s_faces_sequence)
        o_faces_set.erase(o_faces_set.begin() + ks); /// !!! Delete its element from the vector decreasing its size

    double crack_fraction = 0.0;
/// PARTICULAR VALUES FOR FACE ENERGIES ---> MUST GO TO THE "kinematic_fracture_data" FILE
...
double nrm = 1.0*sface_energy_matrix, nrr = 0, ncm = 2.0*sface_energy_matrix, ncr = 2.0*sface_energy_inclusion; // nrm and nrr - coefficients for energies caused by INCLUSIONS in a matrix (m) or inclusion (r) boundary; ncm and ncr - coefficients for energies caused by CRACKS in a matrix (m) or inclusion (r) boundary
 // double  lsc_inclusion = 4.0 * sface_energy_inclusion, lsc_crack = 2.0 * sface_energy_inclusion; // energy values related with the stress concentrators
    ...
    for (unsigned int fn = 0; fn < CellNumbs.at(cell_type); ++fn) {
        ...
        Face_inclusion_index.at(fn) = (j_types_neigh_fractions.at(0) + 2.0 * j_types_neigh_fractions.at(1) + 3.0 * j_types_neigh_fractions.at(2));
...
//        lsc_crack_energy.at(os) = ncm * (Face_crack_index.at(os)/ Face_weight.at(os));
    ...
    for (unsigned int f = 0; f < CellNumbs.at(2); ++f)
        Cell_energy.at(f) = Cell_energy.at(f) -= lsc_inclusion_energy.at(f);
    ...
    double fractured_cells_fraction = 0; /// starting
/// Possible initial pre-fractured state --> S_cVector
    if (Configuration_cState.at(cell_type + (dim - 3)).size() > 0)
        S_cVector = Configuration_cState.at(cell_type + (dim - 3)); // initial predefined system, if exists
// pre-initial 0-fractured state
    for( unsigned int lit = 0; lit < CellNumbs.at(2); lit++)
...
// calculation of the total max cracked cell fraction
    double total_max_cCells_fraction = 0;
    for (int j = 0; j < max_cfractions_vectors[cell_type + (dim - 3)].size(); ++j)
        if(max_cfractions_vectors[cell_type + (dim - 3)][j] > 0)
            total_max_cCells_fraction += max_cfractions_vectors[cell_type + (dim - 3)][j];

    if (total_max_cCells_fraction > 1.0) {
        cout << "WARNING! [Processing_Random()]: "s << cell_type <<" total_max_sCell_fraction of " << cell_type << "-cells in the processing.ini file = " << total_max_cCells_fraction << " that is GREATER than 1 (!) Please decrease the fractions accordingly." << endl;
        Out_logfile_stream << "WARNING! [Processing_Random()]: "s << cell_type <<" total_max_sCell_fraction of " << cell_type << "-cells in the processing.ini file = " << total_max_cCells_fraction << " that is GREATER than 1 (!) Please decrease the fractions accordingly." << endl;
    }
    else if (total_max_cCells_fraction == 0.0) return crack_faces_sequence;

// initial fractions of cracked cells
    double undamaged_cells_fraction = NotFracturedCellNumbs.size()/ (double) CellNumbs.at(cell_type + (dim - 3));
    double special_cells_fraction = 1.0 - undamaged_cells_fraction; // special face vecror definition based on the ordinary face vector
...
//            lsc_inclusion_energy.at(os) = nrm * (Face_inclusion_index.at(os) / Face_weight.at(os));
        ...
        for (unsigned int f = 0; f < CellNumbs.at(2); ++f) // updating current cell's local elastic energy
            Cell_current_energy.at(f) = Cell_energy.at(f) - lsc_crack_energy.at(f);
...
/// Number of the next fractured cell
        ...
        if (find(crack_faces_sequence.begin(), crack_faces_sequence.end(), NewCrackNumb) == crack_faces_sequence.end()) {
            crack_faces_sequence.push_back(NewCrackNumb); // if the element is not in the sequence - add it
            S_cVector.at(NewCrackNumb) = 1;
            NotFracturedCellNumbs.erase(NotFracturedCellNumbs.begin() + NewCrackNumb); // !!! Delete its element from the vector decreasing its size BUT
            Cell_energy.at(NewCrackNumb) = 1000.0; /// arbitrarily large value >> 1 !!!
        } // end if
...
////////
        ...
        TJsCrackTypes = EdgesTypesCalc(CellNumbs, crack_faces_sequence, FES);
        ...
        if (newCrack_neigh_Faces.size() > 0) {
            for (auto fn: newCrack_neigh_Faces) {
                ...
                Face_crack_index.at(fn) = (j_types_crack_n_fractions.at(0) + 2.0 * j_types_crack_n_fractions.at(1) +
                                           3.0 * j_types_crack_n_fractions.at(2));
