...
extern std::vector<std::tuple<double, double, double>> face_coordinates_vector;
 ...
 * @param face_energies
 * @param crack_mode
 * @param new_crack_length
 * @param external_vonMizes_stress
 * @param Puasson_coeff
 * @param Young_modulus
 */
void crack_modes_stress_field(std::vector <double> &face_energies, int crack_mode, double new_crack_length, double external_vonMizes_stress, double Puasson_coeff, double Young_modulus) {
    ...
    for (unsigned int itr = 0; itr < face_coordinates_vector.size()-1; ++itr) { // loop over all GBs and their barycentic coordinates
        double z = get<0>(face_coordinates_vector.at(itr)) * std::get<0>(sample_dimensions);
        double x = get<1>(face_coordinates_vector.at(itr)) * std::get<1>(sample_dimensions);
        double y = get<2>(face_coordinates_vector.at(itr)) * std::get<2>(sample_dimensions);
        ...
        switch (crack_mode) {
            case 1: { // crack mode I
                ...
                double y2p = pow(y, 2.0) + pow(Len, 2.0); ////// y ??
                double y2m = pow(y, 2.0) - pow(Len, 2.0); ////// y ??
