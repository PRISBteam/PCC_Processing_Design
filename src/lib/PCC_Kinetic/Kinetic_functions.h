///================================ A part of the PCC Kinetic module =============================================================///
///=================================================================================================================================///
/** The library contains random and non-random functions for choice and generation of new identifications for k-Cells               *
 *  in the PCC Kinetic module. It makes them "special" and takes out of the set of "ordinary" k-Cells.                            **/
///================================================================================================================================///

/// #1# Macro crack function
/*
...
vector<double> Macrocrack_growth(double max_size_ratio, subcomplex crack_plane_subcomplex) {
    ...
    for (int growth_step = 0; growth_step < number_of_growth_steps; ++growth_step) {
        ...
        int axis_id = 1; // 1 -> x
        std::vector<unsigned int> full_plane_grains = DCC_Plane_cut(a_coeff, b_coeff, c_coeff, D_coeff);
...
/// #3# Wear (thin films plasticity) function
 void DCC_Kinetic_Wear(double ShearStress, vector<Tup> &Grain_Orientations, Eigen::SparseMatrix<double> const& FES) {
    ...
    char* indir = const_cast<char*>(source_dir.c_str()); // const_cast for output directory

/// Vectors
    /// Normals - reading from file
    string Norm_path = source_dir + "Normal.txt"s;
...
/// #2# Plasticity (thin films plasticity) function
...
/// Variables
    ...
    string Norm_path = source_dir + "normals.txt"s;
    char* normals = const_cast<char*>(Norm_path.c_str());
    // Calculation of the NormalForce for each Face
    ...
    string SAreas_path = source_dir + "faces_areas.txt"s;
