...
int output_counter = 0; // special counter for output numeration
    ...
    if(writer_specifications.at(8) == 1)
        PCC_Polyhedrons_Writer(pcc_processed, output_counter);
    cout << "5" << endl;

    if(writer_specifications.at(9) == 1 || writer_specifications.at(10) == 1)
        PCC_Faces_Writer(pcc_processed, output_counter);
    cout << "6" << endl;
